...
export async function POST(request: NextRequest) {
  ...
  const rl = await rateLimit({ key: `upload-logo:${ip}`, limit: 10, windowMs: 60_000 })
  if (!rl.success) {
  ...
  // Validate
  const validation = validateBrandingFile(buffer, "logo")
  if (!validation.ok) {
  ...
  if (settings?.siteLogoUrl) {
    await deleteFile(settings.siteLogoUrl)
  }
