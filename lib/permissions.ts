...
// Cached version returns a plain array (Sets are not JSON-serialisable).
// Keyed per userId; TTL 120 s. Invalidate with revalidateTag("user-permissions").
const fetchUserPermissions = unstable_cache(
  ...
  ["user-permissions"],
  { revalidate: 120, tags: ["user-permissions"] }
)
 ...
 *
 * Results are cached per userId for 120 s. Invalidate with revalidateTag("user-permissions").
 */
